...
class TeamController extends Controller
{
    ...
    public function index(Request $request){
        $team = $this->team->orderBy('pinned', 'desc')->orderBy($request->sortby, $request->sortbydesc) 
        ->when($request->q, function($team) use ($request) { 
            $team = $team->where('title', 'LIKE', '%' . $request->q . '%')
    ...
    public function pinned($id,$type='pinned'){
        ...
        $data = [];
        if($type == 'pinned'){ 
            $this->team->where('pinned', '1')->update([
                'pinned' => '0',
            ]);
            $data = [
                'pinned' => 1,
    ...
    {
        $check = $this->team->where('id', '=', $id)->first();
    ...
    public function store(Request $request){ 
        ...
        $check = $this->team->find($request->id_team);
        if ($check) {
            $imgName = $check->image;

<template>
    <div>
      <div class="d-flex"> 
          ...
          <div class="ms-auto">
              <button class="btn btn-primary" @click="formservice"><i class="bi bi-plus"></i> Add Service</button>
          </div>
      </div>
      ...
      methods: {  
          ...
          handleDelete(val){ 
            ...
            this.$swal({
                ...
                if (result.value) { 
                  ...
                  let stat = 'error',
                      msg = 'Failed delete service';
                    this.$axios.post("api/services/" + val.id, formData)
                      .then((response) => { 
                            if(response.data.status){
                              this.deleted = false; 
                              this.loadPostsData();
          ...
          savePin(){ 
            ...
            let formData = new FormData();
                  ...
                  this.$axios.post("api/services/pinned/" + this.row.id + '/' + this.selectpin, formData)
                  .then((response) => { 
                      this.ispinned = false; 
                      if(response.data.status){
                        this.deleted = false; 
          ...
          handleUnset(val, type){
            ...
            this.$swal({
                ...
                if (result.value) { 
                  ...
                  formData.append('_method', 'PUT'); 
                  formData.append("_token", token);  
                    this.$axios.post("api/services/unset/" + val.id, formData)
                      .then((response) => { 
                            if(response.data.status){
                              this.deleted = false; 
                              this.loadPostsData();

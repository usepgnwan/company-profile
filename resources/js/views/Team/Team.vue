<template>
  <div>
    <div class="d-flex"> 
        ...
        <div class="ms-auto">
            <button class="btn btn-primary" @click="formtestimoni"><i class="bi bi-plus"></i> Add Team</button>
        </div>
    </div>
...
export default { 
    ...
    data() {
      return {
        ...
        fields: [
                    ...
                    {key: 'ig', sortable: false},
                    {key: 'fb', sortable: false},
                    {key: 'linkedin', sortable: false},
    ...
    methods: {  
        ...
        formtestimoni(){
            this.$router.push({ path : "team/form"});
        },
        truncatedText(arr) {
        ...
        handleDelete(val){ 
          ...
          this.$swal({
              ...
              if (result.value) { 
                ...
                let stat = 'error',
                    msg = 'Failed delete team';
                  this.$axios.post("api/team/" + val.id, formData)
                    .then((response) => { 
                          if(response.data.status){
                            this.deleted = false; 
                            this.loadPostsData();
        ...
        savePin(){ 
          ...
          let formData = new FormData();
                ...
                this.$axios.post("api/team/pinned/" + this.row.id + '/' + this.selectpin, formData)
                .then((response) => { 
                    this.ispinned = false; 
                    if(response.data.status){
                      this.deleted = false; 
        ...
        handleUnset(val, type){
          ...
          this.$swal({
              ...
              if (result.value) { 
                ...
                formData.append('_method', 'PUT'); 
                formData.append("_token", token);  
                  this.$axios.post("api/team/unset/" + val.id, formData)
                    .then((response) => { 
                          if(response.data.status){
                            this.deleted = false; 
                            this.loadPostsData();

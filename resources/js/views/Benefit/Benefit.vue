<template>
    <div>
      <div class="d-flex"> 
          ...
          <div class="ms-auto">
              <button class="btn btn-primary" @click="formbenefit"><i class="bi bi-plus"></i> Add Benefit</button>
          </div>
      </div>
  ...
  export default { 
      ...
      methods: {  
          ...
          formbenefit(){
              this.$router.push({ path : "benefit/form"});
          },
          truncatedText(arr) {
          ...
          handleDelete(val){ 
            ...
            this.$swal({
                ...
                if (result.value) { 
                  ...
                  let stat = 'error',
                      msg = 'Failed delete benefit';
                    this.$axios.post("api/benefits/" + val.id, formData)
                      .then((response) => { 
                            if(response.data.status){
                              this.deleted = false; 
                              this.loadPostsData();
          ...
          savePin(){ 
            ...
            let formData = new FormData();
                  ...
                  this.$axios.post("api/benefits/pinned/" + this.row.id + '/' + this.selectpin, formData)
                  .then((response) => { 
                      this.ispinned = false; 
                      if(response.data.status){
                        this.deleted = false; 
          ...
          handleUnset(val, type){
            ...
            this.$swal({
                ...
                if (result.value) { 
                  ...
                  formData.append('_method', 'PUT'); 
                  formData.append("_token", token);  
                    this.$axios.post("api/benefits/unset/" + val.id, formData)
                      .then((response) => { 
                            if(response.data.status){
                              this.deleted = false; 
                              this.loadPostsData();

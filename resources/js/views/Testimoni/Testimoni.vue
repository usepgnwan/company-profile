<template>
    <div>
      <div class="d-flex"> 
          ...
          <div class="ms-auto">
              <button class="btn btn-primary" @click="formtestimoni"><i class="bi bi-plus"></i> Add Testimoni</button>
          </div>
      </div>
      ...
      <!-- modal pinned / highlight -->
        <b-modal v-model="ispinned">
              <template v-slot:modal-header>
                   <h5 class="modal-title">Pinned</h5><button @click="ispinned=false" class="btn btn-sm"><i class="ri-close-line"></i></button>
              </template>
  ...
  export default { 
      ...
      methods: {  
                ...
                .catch ((err) => {
                    ...
                    $('.d-tbale').find('table').find('tbody').html(`
                        <tr class="text-center loading-tr">
                          <td colspan='6'>Error fetching data ${err.message} </span></td>
                        </tr>
                      `);
                })
          ...
          formtestimoni(){
              this.$router.push({ path : "testimoni/form"});
          },
          truncatedText(arr) {
          ...
          handleDelete(val){ 
            ...
            this.$swal({
                ...
                if (result.value) { 
                  ...
                  let stat = 'error',
                      msg = 'Failed delete testimoni';
                    this.$axios.post("api/testimoni/" + val.id, formData)
                      .then((response) => { 
                            if(response.data.status){
                              this.deleted = false; 
                              this.loadPostsData();
          ...
          savePin(){ 
            ...
            let formData = new FormData();
                  ...
                  this.$axios.post("api/testimoni/pinned/" + this.row.id + '/' + this.selectpin, formData)
                  .then((response) => { 
                      this.ispinned = false; 
                      if(response.data.status){
                        this.deleted = false; 
          ...
          handleUnset(val, type){
            ...
            this.$swal({
                ...
                if (result.value) { 
                  ...
                  formData.append('_method', 'PUT'); 
                  formData.append("_token", token);  
                    this.$axios.post("api/testimoni/unset/" + val.id, formData)
                      .then((response) => { 
                            if(response.data.status){
                              this.deleted = false; 
                              this.loadPostsData();
                      ...
                      .catch(() => {
                        this.$swal('Failed', type +' has been deleted.', stat) 
                      });
                }
              });
          }
